...
for secao in range(numero_secoes):
    for new_cs in range(102):
        ...
        if (project.casing_strings[cs_id].interval == 'PRODUCTION'
            ...
            #data['well_strings'][cs_id]['string_sections'][secao]['pipe']['od'] = 10.75
                ...
                for j in range(profundidade_carregamento):
                    ...
                    if base_j <= base and base_j >= topo:
                        ...
                        if Pe < Pi and result2.beta < 4.26: #Ruptura
                            passou = False
                            break
                        if result3.beta < 3.09: #Axial
                            passou = False
                            break
                        if result1.beta < 3.09 and selecao_carregamento[k].tipo_carga == 'Serviço': #von mises
                            passou = False
                            break
